...
static u32 remove_handled_rx_packet(struct sdio_channel *ch)
{
	...
	if (!list_empty(&ch->rx_size_list_head)) {
		...
		pr_info(MODULE_NAME ":%s: ch %8.6s: unexpected empty list!!\n",
			__func__, ch->name);
	}
...
int sdio_read(struct sdio_channel *ch, void *data, int len)
{
	...
	if (ret) {
		...
		pr_err(MODULE_NAME ":ch %8.6s: sdio_read err=%d, len=%d, "
		...
		struct sdio_channel *ch, int n_print_time)
{
	...
	if (print_time) {
		...
		if (strcmp(ch->name, "SDIO_CIQ") == 0) {
			pr_info(MODULE_NAME ": %d <%-9.6s> t=%d, r=%d, tt=%d, rt=%d,"
			...
			pr_info(MODULE_NAME ": %d <%-9s> t=%d, r=%d, tt=%d, rt=%d,"
				" %d-%d, ad%d-%ld(%02d-%02d %02d:%02d:%02d)\n",
				dev->card->host->index, ch->name,
				/* ch statistics */
				ch->total_tx_bytes, ch->total_rx_bytes,
