...
static void sdio_tty_work_func_ciq(struct work_struct *work)
{
	...
	for (;;) {
		...
		if (avail && ptr) {
			...
			/* ++SSD_RIL:Mars@20110830: remove un-useful garbage characters in log file. */
			/* else
				pr_info("[sdio_tty]sdio_tty_work_func_c: read bytes data %s \n", ptr); */
			/* --SSD_RIL */
			wake_lock_timeout(&info->wake_lock, HZ / 2);
			tty->low_latency = 1;
			tty_flip_buffer_push(tty);
...
static int __init sdio_tty_init(void)
{
	...
	/* In vigor, we only use sdio_tty in mfgkernel mode*/
	if (machine_is_vigor() && board_mfg_mode() != 6 && board_mfg_mode() != 7)
		return 0;
